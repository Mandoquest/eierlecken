...
class MessageLimitButtons(discord.ui.View):
    ...
    async def second_button_callback(
        ...
        await interaction.response.send_message(
            "Message limit set to Light: **10 messages every 15 seconds**.",
            ephemeral=True,
        )

    @discord.ui.button(
        label="Medium (Recommended)", style=discord.ButtonStyle.primary, emoji="🔸"
    )
    async def third_button_callback(
        ...
        await interaction.response.send_message(
            "Message limit set to Medium: 5 messages every 10 seconds.", ephemeral=True
        )
    ...
    async def fourth_button_callback(
        ...
        await interaction.response.send_message(
            "Message limit set to Light: **3 messages every 5 seconds**.",
            ephemeral=True,
        )
    ...
    async def fifth_button_callback(
        ...
        await interaction.response.send_message(
            "Message limit disabled.", ephemeral=True
        )
